/**
 * M6 — Envoi d'un document par e-mail / SMS : dépose une notification 'pending'
 * (traitée par l'Edge Function `dispatch-notifications` via pg_cron) et historise
 * l'envoi dans `communications` (onglet CRM de la fiche client).
 */
...
export async function enqueueDocumentEmail(p: {
  ...
  const { error } = await supabase.from('notifications').insert({
    company_id: p.companyId, channel: 'email', to_address: p.contact.email,
    subject: p.subject, body: p.body, entity_type: 'document', entity_id: p.document.id,
