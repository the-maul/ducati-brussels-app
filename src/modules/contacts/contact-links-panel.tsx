...
export function ContactLinksPanel({ companyId, contact }: { companyId: string; contact: Contact }) {
  ...
  const create = useMutation({ mutationFn: () => createLinkedContact(companyId, contact, targetType), onSuccess: (id) => { refresh(); navigate({ to: '/clients/$contactId', params: { contactId: id } }); } });
    ...
    <div className="space-y-3 rounded-md border border-border bg-card p-4 shadow-[var(--shadow-card)]">
      ...
      <div className="space-y-1.5">
        <div className="relative">
          ...
          <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder={t('contacts.linkSearchPlaceholder')} className="pl-8" />
        </div>
        {q.trim().length >= 2 && (
