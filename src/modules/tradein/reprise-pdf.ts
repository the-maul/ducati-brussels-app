...
type Img = { dataUrl: string; format: 'JPEG' | 'PNG'; w: number; h: number };

/** Charge une image (URL signée) → dataURL + dimensions ; null si illisible. */
async function loadImage(url: string): Promise<Img | null> {
  try {
    ...
    // HEIC & co : jsPDF ne gère que JPEG/PNG — reconvertit via canvas si besoin
    if (!dataUrl.startsWith('data:image/png') && !dataUrl.startsWith('data:image/jpeg')) {
      const canvas = document.createElement('canvas');
      canvas.width = bmp.width; canvas.height = bmp.height;
      canvas.getContext('2d')?.drawImage(bmp, 0, 0);
      return { dataUrl: canvas.toDataURL('image/jpeg', 0.88), format: 'JPEG', w: bmp.width, h: bmp.height };
    }
    return { dataUrl, format, w: bmp.width, h: bmp.height };
  } catch { return null; }
}
...
export async function downloadRepriseSheetPdf(s: RepriseSheet): Promise<void> {
  ...
  const renderPhotos = async (title: string, items: RepriseSheetPhoto[]) => {
    ...
    for (const p of items) {
      ...
      try { doc.addImage(img.dataUrl, img.format, x, y, w, h); } catch { continue; }
      y += h + 2;
