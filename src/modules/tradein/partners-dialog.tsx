/**
 * M7 — Gestion des marchands partenaires (occasions) + mode d'envoi.
 * Ajout d'un marchand (nom, e-mail, marques suivies) et bascule du mode
 * de diffusion (auto / semi-auto). Ouvert depuis la page Reprises.
 */
...
import {
  listPartners, addPartner, deletePartner, checkPartnersSchema,
  getDispatchMode, setDispatchMode, type DispatchMode,
} from './partners-api';
...
export function PartnersDialog({ open, onOpenChange, companyId }: { open: boolean; onOpenChange: (o: boolean) => void; companyId: string }) {
  ...
  const dispatchQ = useQuery({ queryKey: ['tradein-dispatch', companyId], queryFn: () => getDispatchMode(companyId), enabled: open });

  const add = useMutation({
    mutationFn: () => addPartner(companyId, { name, email, brands: [...brands] }),
    onSuccess: () => { setName(''); setEmail(''); setBrands(new Set()); qc.invalidateQueries({ queryKey: ['tradein-partners', companyId] }); },
  ...
  const setMode = useMutation({
    mutationFn: (m: DispatchMode) => setDispatchMode(companyId, m),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['tradein-dispatch', companyId] }),
  });

  const partners = partnersQ.data ?? [];
  const mode = dispatchQ.data ?? 'semi_auto';
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <span className="text-[13px] font-bold">{t('tradein.dispatchMode')}</span>
          <Select value={mode} onValueChange={(v) => setMode.mutate(v as DispatchMode)}>
              ...
              {partners.map((p) => (
                <div key={p.id} className="flex items-center justify-between gap-2 rounded-md border border-border px-3 py-2 text-[13px]">
                  <div className="min-w-0">
                    <p className="font-bold">{p.name} <span className="font-normal text-muted-foreground">· {p.email}</span></p>
                    <p className="truncate text-[12px] text-muted-foreground">
                      <Mail className="mr-1 inline size-3" />{p.brands.length === 0 ? t('tradein.partnerAllBrands') : p.brands.join(', ')}
                    </p>
                  </div>
        ...
        <div className="rounded-md border border-dashed border-border p-3">
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
            ...
            <Input placeholder={t('tradein.partnerEmail')} type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
          </div>
