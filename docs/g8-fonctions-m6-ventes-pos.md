...
- [ ] **Mode HT / TTC** : bascule la saisie/affichage — TVA/pro = HT, particulier = TTC (config départ) (p.31)
...
- [ ] **Choix du mode de règlement** (flèches) ; revenir au tableau via ÉCHAP (p.39)
- [ ] **Règlements multiples** : plusieurs modes pour une même facture (montant par mode) (p.39, p.68)
- [ ] **Rendu de monnaie** : montant donné par le client → **monnaie à rendre** calculée (espèces) (p.40, p.71)
...
- [ ] **Fonctions avancées client** (p.45-56) :
  ...
  - [ ] **Frais de port** : montant HT, **Port taxé / Port non taxé**, taux & montant TVA sur port (p.52-54)
  - [ ] **Détaxe** : inhibe la TVA (vente HT / export) — **uniquement en mode HT**, port à passer non taxé (p.54)
...
- [ ] Écran **Pied de facture** : Total TTC, Net HT, TVA, Net TTC, frais de port (p.66)
- [ ] **Remise globale** en **pourcentage** ou en **montant** (n'affecte pas le port) (p.66-67)
- [ ] **Montant net TTC** modifiable (arrondi de facture) → TVA recalculée (p.67)
- [ ] **Saisie des règlements immédiats** : code mode + montant (p.67-68)
- [ ] **Modes de règlement** (table configurable) : ESPECES, CHEQUE, CARTE BLEU, TRAITE, LCR, CREDIT/LEASING, VIREMENT (p.68)
- [ ] **Règlement total** ou **partiel** (un mode), **multi-modes** (plusieurs lignes d'encaissement) (p.68)
- [ ] **Encaissements à échéances** : règlement non perçu de suite (chèque/LCR différé) → alimente le journal de caisse à la date d'échéance, client reste débiteur jusqu'au perçu (p.69)
- [ ] **Modification / suppression / ajout** d'un encaissement avant validation (p.70)
...
- [ ] Saisie identique à la facture (réfs, quantités) ; débite stock **disponible** uniquement (p.74)
- [ ] **Saisie d'acompte** : optionnel ou montant ; mode de règlement de l'acompte ; **plusieurs acomptes** possibles (p.75-76)
- [ ] Échéances sur l'acompte (p.75)
