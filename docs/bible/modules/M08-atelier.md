...
| Vues pour l'écran parcours technicien (M-33) | `ducati_manual_programs` (une ligne par modèle-année couvert), `ducati_manual_procedures` (avec `etapes`), `ducati_manual_procedure_steps` — formes de `src/modules/workshop/journey/types.ts`, `security_invoker` pour garder la RLS des tables `wsm_*` |
...
| 2026-09-23 | **Manuels d'atelier Ducati** (mission 07 carte 3, ATE014) : 15 tables `wsm_*`, chargeur `tools/wsm-loader`, onglet « Manuels d'atelier ». **Chargé en production** : 461 manuels couvrant 469 modèles-années, 3 336 procédures, 36 326 étapes | `20260923100000_m8_manuels_atelier.sql`, `20260923101000_m8_manuels_atelier_chargement.sql` (appliquées le 23/09) |
| 2026-09-23 | Un manuel peut couvrir plusieurs modèles-années (M-31) ; règle de rapprochement des noms alignée sur le chargeur (418 rattachements au lieu de 253) ; trois vues `ducati_manual_*` pour l'écran parcours technicien (M-33) | `20260923102000_m8_manuels_atelier_modeles_couverts.sql`, `20260923103000_m8_manuels_atelier_vues_parcours.sql` (**non appliquées**) |
