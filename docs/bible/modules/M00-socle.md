...
missions: []
mots_cles: [connexion, login, mot de passe, invitation, utilisateur, rôle, admin, société, multi-société, RLS, audit, events, numérotation, séquence, préfixe, paramètres, tables de données, recherche globale, Ctrl+K, sidebar, topbar, cloche, i18n, traduction, améliorations]
---
...
| Barre latérale | 19 entrées (`src/lib/navigation.ts`). Seules Comptabilité (admin, comptable), Améliorations (admin) et Paramètres (admin) sont filtrées par rôle. |
...
| Améliorations (admin) | Tableau de tâches partagé entre administrateurs, avec sous-points et pièces jointes. |
...
| Améliorations | `src/modules/improvements/` |
...
| Tables | `companies` (sociétés), `profiles` (profil de chaque compte), `user_roles` (rôle par société), `events` (journal d'audit, 200 000+ lignes), `document_sequences` (28 séquences), `reference_values` (144 valeurs), `contact_accounts` (compte client ↔ fiche), `improvements`, `improvement_points` |
...
| Migrations clés | `supabase/migrations/20260610090000_m0_socle.sql`, `20260610100000_m0_bootstrap_admin.sql`, `20260610180000_m0_reference_tables.sql`, `20260610280000_m6_fix_sequences.sql`, `20260610420000_m0_companies_mgmt.sql`, `20260612330000_m0_company_logo.sql`, `20260612900000_m0_improvements.sql`, `20260918140000_m10_users_pipelines_mailbox_summary.sql` (`contact_accounts`) |
| Libellés | `src/lib/i18n/fr.ts`, blocs `app`, `common`, `nav`, `action`, `feedback`, `confirm`, `company`, `search`, `status`, `auth`, `settings`, `users`, `pwd`, `role`, `improvements`, `demo` ; point d'entrée `src/lib/i18n/index.ts` |
...
- 🔴 **Fonctions exécutables sans être connecté.** 93 des 106 fonctions `SECURITY DEFINER` sont exécutables par le rôle `anon`. Pour le socle, les plus graves :
  ...
  - Le `company_id` n'est pas secret : il apparaît dans `shop_public_info` / `shop_public_site` dès qu'une boutique est publiée, et dans les URL des images du bucket public `shop-assets`.
...
- **Tout salarié voit tout** dans sa société : la RLS ne distingue pas les rôles, sauf pour les tables d'administration (`companies`, `user_roles`, `document_sequences` en écriture, `reference_values` en écriture, `improvements`). Un mécanicien peut lire la comptabilité par l'API même si le menu la lui cache.
