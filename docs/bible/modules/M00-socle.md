...
missions: []
mots_cles: [connexion, login, mot de passe, règles de mot de passe, invitation, mail de bienvenue, borne, kiosque, QR code, utilisateur, rôle, admin, société, multi-société, RLS, audit, events, numérotation, séquence, préfixe, paramètres, tables de données, recherche globale, Ctrl+K, sidebar, topbar, cloche, i18n, traduction]
---
...
- **Cloche par rôle et par responsable (N-1, 19/09)** : tâches CRM confiées à la personne (admin : + sans responsable, bascule équipe) ; inscriptions → vendeur, marketing, admin ; demandes de rendez-vous du portail → mécanicien, chef d'atelier, admin. La lecture des inscriptions est refusée en base aux autres rôles (`can_see_team_notification`, politique `team_notifications_member_read`).
