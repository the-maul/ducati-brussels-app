...
| Tests | `tests/sales-totals.test.ts` (8 cas : totaux, pied, détaxe, net forcé), `tests/sales-doc-chain.test.ts` (chaîne DEV → BC → RES/BL/FAC), `tests/sales-stock-status.test.ts` (disponible / en commande / à commander), `tests/sales-replacement-chain.test.ts` (chaîne de remplacement) |
