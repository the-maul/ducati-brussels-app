...
- **Disponible affiché** (B4) = réel − réservé (fonction existante `article_stock`) **+ en commande** = quantités des commandes fournisseur **CMD validées** sans réception **reçue** liée (`source_order_id`). Les commandes de pièces ne sont pas comptées (elles le seront une fois transformées en CMD, carte 4).
