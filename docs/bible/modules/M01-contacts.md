...
| Espace client → **Mon profil** (`/mon-espace/profil`) | (retour client du 21/09) **E-mail** dans un champ à part, en lecture seule (c'est l'identifiant de connexion, explication sous le champ) ; **GSM** et **téléphone fixe** avec menu de préfixe pays (+32 par défaut) ; **permis** : deux photos, **recto** et **verso**, chacune « Prendre une photo » ou « Choisir un fichier », avec l'état « Déposé / Pas encore déposé ». Accueil `/mon-espace` : carte « Complétez votre profil » avec **« 50 % complété »**, barre colorée (orange < 50 %, bleu, vert à 100 %), « Encore N étapes : finalisez votre profil… » et bouton **« Finaliser mon profil »** vers la première étape à faire. |
...
| Téléphone avec indicatif | `src/components/phone-input.tsx`, `src/lib/dial-codes.ts` ; **format unique** (21/09) : `src/lib/phone.ts` (`toE164`, `formatPhone`, `formatLocalPart`, `isValidPhone`) ; écran Mon profil : `src/modules/portal/profile.tsx` (`LicenseSide` pour le recto/verso du permis) ; carte de complétion : `src/modules/portal/portal-home.tsx`, `ProgressBar` / `progressTone` dans `src/modules/portal/ui.tsx` |
...
- **Téléphones : un seul format (retour client du 21/09)** : partout (inscription `/inscription` et borne, espace client, fiche contact du personnel), saisie par **menu de préfixe pays (+32 par défaut) + numéro national** (`PhoneInput`), enregistrement au format **E.164** (`+32470123456`, fonction `toE164`), affichage lisible `+32 470 12 34 56` (`formatPhone` : liste des clients, parc d'une moto, rapprochement CRM). « 0470… », « 0032… » et « +32 0470… » deviennent `+32470…`. Une saisie qui contient autre chose que des chiffres et `+ ( ) . / -` (lettres, « @ ») est **refusée** : message sous le champ, et refus au serveur (inscription).
- **Jamais d'e-mail dans un téléphone (21/09)** : cause du bug de la vidéo (« +32 desouter.d@gmail.com » dans le GSM) : l'inscription acceptait n'importe quel texte comme téléphone et le gestionnaire de mots de passe du navigateur y avait écrit l'identifiant enregistré (le champ e-mail ne portait pas `autocomplete="username"`). Corrigé à l'écran, au serveur et **en base** : le déclencheur `trg_contacts_phone_not_email` (avant insertion ou modification de `mobile`, `gsm`, `phone`) déplace une adresse vers l'e-mail s'il est vide, sinon vide le champ, et l'écrit dans `events` (`contact_phone_email_guard`). Les fiches existantes sont réparées une fois par la migration `20260921160000` (`contact_phone_email_repair`, une ligne par fiche, anciennes valeurs conservées).
- **Permis recto / verso (21/09)** : dépôt `permis` = recto (type existant), `permis_verso` = verso ; même mécanisme que les autres dépôts du portail (`portal_prepare_upload` → bucket `ged` → `portal_complete_upload`, visible par l'équipe dans la GED de la fiche, dossier « Portail client »). L'étape « Votre permis de conduire » de l'accueil est faite avec le **numéro de permis** ou les **deux photos**.
...
- **Téléphone invalide dans le formulaire du personnel** : `PhoneInput` affiche « Ce n'est pas un numéro de téléphone », mais l'enregistrement de la fiche n'est pas bloqué (la valeur est ignorée par `normalizeMobile`, la base refuse de toute façon un « @ »).
- **Migration `20260921160000`** : redéfinit `portal_home` et `portal_prepare_upload` d'après `20260919120000`, `portal_profile` d'après `20260919265000`. Si la base a reçu une version plus récente hors dépôt, reporter la différence avant d'appliquer.
...
| 2026-09-19 | **Mission 04, cartes 6 à 8** : onglet Parc → « Ajouter une moto » (moto + propriétaire en une transaction, rattachement d'une moto existante si le VIN est connu) ; espace client → « Ajouter ma moto » (déclaration dans `contact_declared_vehicles`, étendue : `vin`, `plate`, `status`, photo de carte grise), validée par l'équipe (voir M03). La fusion de fiches déplace déjà `contact_declared_vehicles` | branche `lot-m4-moto`, migrations `20260919300000`, `20260919302000`, `20260919303000` (appliquées le 19/09) |
| 2026-09-21 | **Retour client « Inscription en ligne »** : e-mail rangé dans le GSM corrigé (cause : inscription sans contrôle du téléphone + remplissage du navigateur) — champ E-mail distinct en lecture seule dans Mon profil, contrôle à l'écran, au serveur et en base, réparation des fiches existantes ; téléphones au format unique E.164 avec préfixe pays (inscription, Mon profil, fiche) ; permis recto + verso ; « 50 % complété » coloré et « Finaliser mon profil » | branche `lot-profil`, migration `20260921160000_m1_telephone_sans_email_permis_verso.sql` (**à appliquer**) |
