...
| Tables | `articles` (fiche + prix courants + PAMP), `article_categories` (familles), `article_barcodes` (codes-barres), `article_suppliers` (fournisseurs secondaires), `article_kit_items` (nomenclature), `article_bins` (multi-casiers, **non utilisée par le code**), `price_changes` (historique des prix, append-only), `article_applicabilities` (compatibilités), `label_templates` (formats d'étiquettes), `article_import_settings` et `ppc_price_rules` (réglages d'import — voir §5), `shopify_products` (instantané des variantes Shopify, écrit par la fonction serveur) et `shopify_links` (décision de liaison variante ↔ article), `shopify_content_imports` (journal de la reprise photos/textes par produit ↔ article, texte Shopify noté à côté) ; `shopify_sync_settings` (mode par société), `shopify_sync_trial` (articles d'essai), `shopify_sync_queue` (file, une ligne par article), `shopify_sync_log` (journal des envois, append-only) ; `shopify_published_media` (photos du DMS déjà envoyées sur un produit) ; colonnes `articles.web_title` / `web_description` ; photos dans `attachments` (M09 : `alt_text`, `sort_order`, `external_id` = id d'image Shopify) |
...
| Fonctions serveur (Edge) | `shopify-sync-products` (lecture de tous les produits Shopify + liaison automatique exacte) ; `shopify-import-content` (reprise photos et textes) ; `shopify-push` (stock + prix vers le site, file, pg_cron 3 min) ; `shopify-publish` (publier / retirer / mettre à jour un article) ; règles pures `supabase/functions/_shared/shopify-push.ts` (prix TTC, disponible, mode, plan, mutations) et accès réseau `_shared/shopify-client.ts` ; règles pures `supabase/functions/_shared/shopify-match.ts` et `shopify-content.ts` (nettoyage HTML, règle « ne pas écraser ») |
| Tâches planifiées | `shopify-push` (toutes les 3 min, n'appelle la fonction que si une société est en Essai / Tous avec une file non vide) |
...
- **Remplacement de référence** (INV012) : `superseded_by_id` ; au remplacement, le stock de l'ancienne réf. passe sur la nouvelle avec son PAMP (`transfer_stock_on_replace`, deux mouvements `transfert` tracés, B7). Une référence peut être remplacée plusieurs fois (chaîne A → B → C) : **en vente, on propose toujours la dernière** de la chaîne (mission 05, carte 3, `src/modules/sales/replacement.ts`, boucles détectées). Au 19/09 : 16 331 références remplacées, 4 969 chaînes d'au moins deux maillons, aucune boucle ; `equivalence_group` toujours vide.
...
- **Le DMS écrit sur Shopify (W-8, 21/09)** : stock disponible (réel − réservé) et PV TTC (arrondi société ; à défaut PV HT × TVA de l'article ; jamais 0 €) des articles reliés, via la file `shopify_sync_queue` alimentée par déclencheurs AFTER sur `stock_moves` (insert), `articles` (PV TTC / PV HT / TVA), `shopify_links`, `companies.round_sale_prices_up`. Lecture seule du stock et des prix du DMS. Mode société Arrêtée (défaut) / Essai / Tous, contrôlé en SQL (`_shopify_push_claim`) et en TS (`writeAllowed`).
...
- **Article « à compléter »** (mission 05, carte 4) : `articles.to_complete` (+ `to_complete_source`, ex. `ecatalog`). Créé depuis une vente quand la référence collée n'existe pas : type **A** en **librairie**, marque Ducati, prix de vente saisi par le vendeur (à l'insertion, comme toute création ; les changements suivants passent par `price_changes`). Le magasin complète PA, fournisseur, famille puis décoche la case. Création et levée tracées dans `events` (`article_to_complete`, `article_completed`, trigger `trg_articles_to_complete`). Recherche exacte d'une référence : `sale_article_exact_lookup`.
...
- **Synchronisation Shopify** : le site peut vendre une pièce avant que la vente n'arrive dans le DMS ; tant que la vente Shopify n'a pas créé sa sortie de stock, un envoi du DMS peut remettre l'ancienne quantité (corrigé au passage suivant). Au 21/09 le DMS n'a **aucun stock** sur les 300 articles reliés et des PV TTC ~20 % sous le site : ne pas passer en « Tous » avant correction.
...
| 2026-09-19 | Articles **« à compléter »** créés au vol depuis une vente (référence e-catalog Ducati collée, mission 05 carte 4) : librairie, type A, marque Ducati ; filtre et badge dans la liste, case sur la fiche ; trace `events` | `20260919340000_m2_article_a_completer_ecatalog.sql` (appliquée le 19/09) |
...
| 2026-09-21 | **Stock et prix du DMS vers le site** (mission 03, W-8) : file dédoublonnée, tâche `shopify-push` 3 min, mode Arrêtée / Essai / Tous (livré Arrêtée), simulation, journal, « Tout resynchroniser » | migration `20260921110000_m2_shopify_push_stock_prix.sql` (appliquée le 21/09), fonction `shopify-push` (déployée le 21/09) |
| 2026-09-21 | **Publier / retirer / mettre à jour un article sur le site** depuis sa fiche (mission 03, W-8), soumis au mode essai | migration `20260921111000_m2_shopify_publication.sql` (appliquée le 21/09), fonction `shopify-publish` (déployée le 21/09) |
