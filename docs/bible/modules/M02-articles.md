...
| Fonctions serveur (Edge) | `shopify-sync-products` (lecture de tous les produits Shopify + liaison automatique exacte) ; `shopify-import-content` (reprise photos et textes) ; `shopify-push` (stock + prix vers le site, file, pg_cron 3 min) ; règles pures `supabase/functions/_shared/shopify-push.ts` (prix TTC, disponible, mode, plan, mutations) et accès réseau `_shared/shopify-client.ts` ; règles pures `supabase/functions/_shared/shopify-match.ts` et `shopify-content.ts` (nettoyage HTML, règle « ne pas écraser ») |
