...
| `send-account-invitation` | Envoie par Outlook le lien pour choisir son mot de passe (jeton à usage unique vers `/reset-password`) | `src/modules/settings/users-api.ts` | `MS_GRAPH_*` (+ clé de service) | oui | réservé à un administrateur de la société |
