...
| `shopify-orders` (mission 03, 21/09) | Commandes du site : webhooks Shopify (signature HMAC du Client secret vérifiée), rattrapage planifié, écran « Commandes du site » (Réessayer, Relire maintenant) ; relit la commande par l'API GraphQL puis `_shopify_order_apply` (facture, stock, règlement, avoirs) | Shopify (webhooks), pg_cron `shopify-orders-catchup`, `src/modules/sales/web-orders-api.ts` | `SHOPIFY_STORE_DOMAIN`, `SHOPIFY_CLIENT_ID`, `SHOPIFY_CLIENT_SECRET`, `CRON_SECRET` (+ clé de service) | non | webhook : signature + domaine ; sinon `x-cron-secret`, clé de service ou utilisateur actif (admin ; vendeur pour Réessayer) ; rien n'est importé tant que le réglage est Arrêté |
...
| `shopify-publish` | Mission 03 : publier (productSet), retirer (brouillon) ou mettre à jour un article sur Shopify depuis sa fiche ; mode `simulate` en lecture seule | fiche article (`src/modules/articles/shopify-publish-panel.tsx`) | `SHOPIFY_*` | non | administrateur actif (ou clé de service) ; refus si mode « Arrêtée » ou article hors essai ; la tâche planifiée ne publie jamais |
...
| `shopify-orders-catchup` | `*/15 * * * *` | `_cron_shopify_orders_tick()` : libère les réservations des commandes du site non payées après N jours (dans la base, depuis `20260921150000`), puis `net.http_post` → Edge Function `shopify-orders` avec l'en-tête `x-cron-secret` (coffre Vault), **seulement si une société a l'import des commandes du site Actif** (livré Arrêté) | `20260921120000_m6_shopify_commandes_site.sql` (21/09), `20260921150000_m6_shopify_reservation_commandes.sql` |
