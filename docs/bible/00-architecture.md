...
| `tools/myducati-extension/` | Extension navigateur qui importe les données « My Ducati » dans le DMS (`public/myducati-extension.zip` = version téléchargeable) |
...
| `shopify-orders-catchup` | `*/15 * * * *` | `net.http_post` → Edge Function `shopify-orders` avec l'en-tête `x-cron-secret` (coffre Vault), **seulement si une société a l'import des commandes du site Actif** (livré Arrêté) | `20260921120000_m6_shopify_commandes_site.sql` (21/09) |
